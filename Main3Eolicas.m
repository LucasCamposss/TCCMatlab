...
% GLOBAL FMINCON
global OBJF DADOSDMR DTEN DVIO MONLT QGEN NUMGERFIC BARGERFIC DADOSFMINCON CPG Bsh
...
ResultLinprog = zeros(Nc,NGER+NBAR,NLIN);
...
% PGW = PGWMAX.*[pout_amazon_wind(ic) pout_High_Sheldon(ic) pout_Timber_road(ic)];
...
violacaoPercentualFluxo = zeros(NLIN,3,NLIN);
for i=1:NLIN
    violacaoPercentualFluxo(:,:,i) = violacao(Max_FLUX(i,:), PB*FLIM2');
end
...
somaGeracao = sum(ResultLinprog(1:end,1:NGER,1),2);
figure
plot(linspace(1,Nc,Nc),PB*ResultLinprog(:,1,1),linspace(1,Nc,Nc),PB*ResultLinprog(:,2,1),linspace(1,Nc,Nc),...
PB*ResultLinprog(:,3,1), linspace(1,Nc,Nc),PB*ResultLinprog(:,4,1), linspace(1,Nc,Nc),PB*ResultLinprog(:,5,1),...
linspace(1,Nc,Nc),PB*somaGeracao(:))

legend(["Gerador 1" "Gerador 2" "Gerador 3" "Corte de carga 1" "Corte de carga 2" "Geração total"])
...
figure
plot(linspace(1,Nc,Nc),PB*PgwGeral(:,1),linspace(1,Nc,Nc),PB*PgwGeral(:,2),linspace(1,Nc,Nc),PB*PgwGeral(:,3),...
    linspace(1,Nc,Nc),PB*somaEolicas(:))
legend(["Eólica 1" "Eólica 2" "Eólica 3" "Geração Total"])
